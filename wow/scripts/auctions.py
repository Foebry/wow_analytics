...
class Auction():
    ...
    def __init__(self, live_data, logger, previous_auctions=None, insert_data=None, update_data=None, request=None, db=None, *args, **kwargs):
        ...
        first_pet = self.item_id == 82800 and self.id not in live_data["items"]
        is_pet = self.item_id == 82800 and self.id in live_data["items"]
        existing_pet = is_pet and self.pet_id["id"] in live_data["items"][self.id]
        new_pet = is_pet and self.pet_id["id"] not in live_data["items"][self.id]
        new_item = not self.item_id == 82800 and self.id not in live_data["items"]
    ...
    def update(self, live_data, previous_auctions, update_data, insert_data, existing, logger):
        ...
        set_realm_update_data_auctions = "auctions" in updata_data and self.realm_id in updata_data["auctions"]
        ...
        if set_realm_update_data_auctions:
            ...
            logger.log(f"Auction not in any category // {vars(self)}")
...
class SoldAuction():
    ...
    def insert(self, insert_data, update_data, logger):
        ...
        partial_set_realm_insert_data_sold_auctions = self.partial and "sold_auctions" in insert_data and realm_id in insert_data["sold_auctions"]
        partial_unset_realm_insert_data_sold_auctions = self.partial and "sold_auctions" in insert_data and realm_id not in insert_data["sold_auctions"]
        ...
        if partial_set_realm_insert_data_sold_auctions or valid_complete_set_realm_insert_data_sold_auctions:
            insert_data["sold_auctions"][realm_id].append(self)
